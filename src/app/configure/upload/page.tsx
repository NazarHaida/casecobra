"use client"

import {cn} from "@/lib/utils";
import {useState, useTransition} from "react";
import Dropzone, {FileRejection} from "react-dropzone";
import {Image, Loader2, MousePointerSquareDashed} from "lucide-react";
import {Progress} from "@/components/ui/progress";
import {useUploadThing} from "@/lib/uploadthing";
import {useRouter} from "next/navigation";
import {underline} from "next/dist/lib/picocolors";

const Page = () => {
    const [isDragging, setIsDragging] = useState(false)
    const [uploadProgress, setUploadProgress] = useState(0)
    const router = useRouter()
    const {startUpload, isUploading} = useUploadThing("imageUploader", {
        onClientUploadComplete: ([data]) => {
            const configId = data.serverData.configId;
            starTransition(()=> {
                router.push(`/configure/design?id=${configId}`)
            })
        },
        onUploadProgress: (progress) => {
            setUploadProgress(progress)
        }
    })
    const onDropRejected = (rejectedFiles: FileRejection[]) => {
        const [file] = rejectedFiles
        setIsDragging(false)


    }
    const onDropAccepted = (acceptedFiles: File[]) => {
        startUpload(acceptedFiles, {configId: undefined})

        setIsDragging(false)
    }

    const [isPending, starTransition] = useTransition()

    return (
        <div
            className={cn(
                'relative h-full flex-1 my-16 w-full rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:rounded-2xl flex justify-center flex-col items-center',
                {
                    'ring-blue-900/25 bg-blue-900/10': isDragging,
                }
            )}>
            <div className='relative flex flex-1 flex-col items-center justify-center w-full'>
                <Dropzone
                    onDropRejected={onDropRejected}
                    onDropAccepted={onDropAccepted} accept={{
                    "image/png": [".png"],
                    "image/jpeg": [".jpeg"],
                    "image/jpg": [".jpg"],
                }}
                    onDragEnter={() => setIsDragging(true)}
                    onDragLeave={() => setIsDragging(false)}
                >
                    {({getRootProps, getInputProps}) => (
                        <div className='h-full w-full flex-1 flex flex-col items-center justify-center'
                             {...getRootProps()}>
                            <input {...getInputProps()}/>
                            {isUploading || isPending ?
                                <MousePointerSquareDashed className={"h-6 w-6 text-zinc-500 mb-2"}/> : false ?
                                    <Loader2 className={"animate-spin h-6 w-6 text-zinc-500 mb-2"}/> :
                                    <Image className='h-6 w-6 text-zinc-500 mb-2'/>
                            }
                            <div className={"flex lfex-col justify-center mb-2 text-sm text-zinc-700"}>
                                {isUploading ?
                                    <div className={"flex flex-col items-center"}>
                                        <p>Uploading...</p>
                                        <Progress value={uploadProgress} className={"mt-2 -40 h-2 bg-gray-300"}/>
                                    </div> : isPending ?
                                        <div className={"flex flex-col items-center"}>Redirecting, please
                                            wait...</div> : isDragging ?
                                            <p><span className={"font-semibold"}>Drop file</span> to upload </p> :
                                            <p><span className={"font-semibold"}>Click to upload</span> or drag and drop </p>}
                            </div>
                        </div>
                    )}

                </Dropzone>
            </div>
            {isPending ? null : <p className={"text-xs text-zinc-500"}> PNG, JPG, JPEG</p>}
        </div>)
}

export default Page